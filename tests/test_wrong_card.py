...
def test_make_reservation_negative_wrong_card_number():
    login()
    search_offers()
    ...
    card_input.clear()
    card_input.send_keys("1111111111111110")
    sleep(10)
    card_input.submit()
...
def test_make_reservation_without_logging_in():
    search_offers()
...
if __name__ == "__main__":
    # set up
    url = 'http://localhost:17574'
    driver = webdriver.Chrome()
    driver.get(url)
    driver.maximize_window()

    # execute user stories
    test_make_reservation_without_logging_in()
    test_make_reservation_negative_wrong_card_number()
